...
export default function Dashboard() {
  ...
  const chartData = useMemo(() => {
    ...
    // Process raw data into monthly series
    const months: any = {};
      ...
      <main className="flex-1 p-4 lg:p-12 overflow-y-auto">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button variant="outline" onClick={handlePdfExport} className="hidden md:flex">
              <Download className="w-4 h-4" />
              PDF
            </Button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
          ...
          <Card className="p-8">
            <div className="flex items-center justify-between mb-8">
              <div>
                <h3 className="text-xl font-black text-slate-900 dark:text-white">Expense Categories</h3>
                <p className="text-sm text-slate-500 font-medium">Top 5 spending areas</p>
              </div>
            ...
            <div className="flex items-center gap-4">
              <div className="relative">
                <input
                  ...
                  className="pl-10 pr-4 py-2 rounded-xl bg-slate-200/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 outline-none focus:ring-2 focus:ring-indigo-500 transition-all text-sm w-64"
                />
                      ...
                      <div key={log.id} className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800">
                        <div className="flex items-center gap-4">
                          <div className={cn(
                            ...
                            log.action === 'DELETE' ? "bg-rose-500/10 text-rose-500" : "bg-indigo-500/10 text-indigo-500"
                          )}>
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-500 font-medium">
                              By {log.user?.email || 'System'} • {new Date(log.createdAt).toLocaleString()}
                            </p>
                          </div>
                        </div>
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-slate-50 dark:divide-slate-900">
                    {isTransactionsLoading ? (
                      [1, 2, 3, 4, 5].map((i) => (
                        <tr key={i}>
                          <td colSpan={4} className="px-8 py-6"><Skeleton className="h-12 w-full" /></td>
                        </tr>
                      ))
                    ) : (
